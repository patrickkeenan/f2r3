...
import { Root, Fullscreen, Container, Text, Image } from "@react-three/uikit";
import { signal } from "@preact/signals-react";
import { Color } from "three";

const SPECIAL_NAME_TOKENS = [/\[.*?\]/, "*"];

const CONTAINER_NODE_TYPES = ["FRAME", "COMPONENT", "INSTANCE", "GROUP"];
...
// export function Scene({ layoutData, imagesByNodeId, rootId, ...props }) {
...
//   // console.log("check doc", layoutData);
//   // if (layoutData.document) {
//   //   console.log("check doc2", layoutData.document);
//   // }
...
//   useEffect(() => {
//     setRootNode(findNodeById(allNodes, rootId));
//   }, [rootId]);
...
export function FigmaLayer({
  ...
  const outerPropsFromNode = figOuterProps(node, parentNode);
  ...
  if (imagesByNodeId[nodeVariant.id]) {
    console.log(
      "trying to render image",
      nodeVariant.id,
      imagesByNodeId[nodeVariant.id]
    );
    innerComponent = (
      <Image
        src={imagesByNodeId[nodeVariant.id]}
    ...
    let path;
    if (nodeVariant.fillGeometry && nodeVariant.fillGeometry[0].path) {
      path = nodeVariant.fillGeometry[0].path;
    } else if (
      nodeVariant.strokeGeometry &&
      nodeVariant.strokeGeometry[0].path
    ) {
...
function figOuterProps(node, parentNode) {
  if (!node) return {};
